...
public class JwtHeaderFilter  extends AbstractGatewayFilterFactory<JwtHeaderFilter.Config> {
    ...
    // Lấy thông tin user từ JWT đã được xác thực ở API Gateway → lấy sub và preferred_username → đưa chúng vào HTTP Header → gửi request tiếp tục xuống microservice.
    // Request cũ
    //     ↓
    ...
    // Mục đích là bên userservice có thể lấy ra user id và user name để thực hiện các thao tác khác
    @Override
